var components = ['input-field', 'select-field', 'left-labeled-input-field', 'tagging-dropdown-field', 'search-field', 'edit-page-item', 'animated-button', 'info-divider', 'checkbox-field'];
...
var options = {
	...
	methods: {
		...
		resetNameOfficeFields: function resetNameOfficeFields() {
			this.emp_id = '';
			this.search_name = '';
			this.office = '';
			this.first_name = '';
		...
		sendFormThroughAjax: function sendFormThroughAjax() {
			...
			if (this.new_employee) form_data.append('new_employee', this.new_employee);else form_data.append('emp_id', this.emp_id);
	...
	computed: {
		completeLabel: function completeLabel() {
			//concatinate all name parts and remove all space
			var name = (this.first_name + ' ' + this.middle_name + ' ' + this.last_name + ' ' + this.suffix.replace('.', '')).replace(/ +/g, "");
