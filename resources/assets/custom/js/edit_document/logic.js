let tagging_field = $('.ui.multiple.search.selection.dropdown');
...
tagging_field.dropdown({
    ...
    onLabelRemove(value){
    	...
    	let index = tagging_field.dropdown('get value').toLocaleLowerCase().indexOf(value);
    	//2. extract the value that has the correct letter casing to be removed using the index and the length of value
    	let exact_value = tagging_field.dropdown('get value').substr(index, value.length);
